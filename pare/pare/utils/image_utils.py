...
import numpy as np
# jpeg4py is optional - use cv2 as fallback
try:
...
def read_img(img_fn):
    ...
    #  return img
    if (img_fn.endswith('jpeg') or img_fn.endswith('jpg')) and JPEG4PY_AVAILABLE:
        try:
            with open(img_fn, 'rb') as f:
                img = np.array(jpeg.JPEG(f).decode())
        except (jpeg.JPEGRuntimeError, Exception):
            # Fallback to cv2 if jpeg4py fails
            img = cv2.cvtColor(cv2.imread(img_fn), cv2.COLOR_BGR2RGB)
    else:
        # Use cv2 for all images if jpeg4py is not available
        img = cv2.cvtColor(cv2.imread(img_fn), cv2.COLOR_BGR2RGB)
