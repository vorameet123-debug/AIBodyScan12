...
# Renderer is optional - may fail on Windows without proper OpenGL setup
try:
...
class PARETester:
    ...
    def _load_pretrained_model(self):
        ...
        ckpt = torch.load(self.args.ckpt, weights_only=False, map_location=self.device)['state_dict']
        ...
        for img_idx, img_fname in enumerate(image_file_names):
            ...
            if not self.args.no_render:
                if not RENDERER_AVAILABLE:
                    logger.warning("Renderer not available, skipping rendering")
                else:
