...
class MeasurementPipeline:
    ...
    def process_image(
        ...
        try:
            ...
            logger.info("Step 1: Running PARE 3D reconstruction...")
            ...
            logger.info(f"Step 2 Result: measurements = {measurements}")
            logger.info(f"  Type: {type(measurements)}")
            logger.info(f"  Length: {len(measurements) if measurements else 'N/A'}")
            
            # Validate that we got at least some measurements
            if not measurements or len(measurements) == 0:
                logger.error("CRITICAL: No measurements were extracted from the 3D mesh!")
                logger.error("This could mean:")
                logger.error("  1. Measurement extraction failed completely")
                logger.error("  2. All measurement attempts failed")
                logger.error("  3. SMPL measurer is not working correctly")
                return {
                    "success": False,
            ...
            logger.info(f"Successfully extracted {len(measurements)} measurements before adding outseam")
            ...
            if calculate_outseam is not None:
                try:
                    ...
                    if outseam is not None:
                        ...
                        logger.info(f"Calculated outseam: {outseam} cm")
                except Exception as e:
            ...
            if self.size_engine is not None and measurements:
                try:
                    logger.info("Step 4: Calculating size recommendations...")
                    size_recommendations = self.size_engine.recommend_all_sizes(
                        measurements,
                        gender=gender,
                        age=age
                    )
                    logger.info(f"Size recommendations calculated for {len(size_recommendations)} categories")
                except Exception as e:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    logger.info(f"Detector returned: {type(detections)}, length: {len(detections) if detections else 0}")
                ...
                logger.info(f"Detected {len(detections)} person(s) in image")
                ...
                try:
                    ...
                    logger.info("PARE inference completed")
                ...
                if not os.path.exists(pare_output_path):
                    ...
                    if os.path.exists(os.path.join(output_path, 'pare_results')):
                        files = os.listdir(os.path.join(output_path, 'pare_results'))
                        logger.info(f"Files in pare_results: {files}")
                    return None
                
                pare_output = joblib.load(pare_output_path)
                logger.info(f"PARE output type: {type(pare_output)}")
                
                # Extract vertices - PARE output uses 'smpl_vertices' key
                if isinstance(pare_output, dict) and 'smpl_vertices' in pare_output:
                    # PARE output format: {' smpl_vertices': array, 'smpl_joints3d': array, ...}
                    verts = pare_output['smpl_vertices']
                    logger.info(f"Extracted smpl_vertices shape: {verts.shape}")
                elif isinstance(pare_output, dict) and 'verts' in pare_output:
                    # Alternative format
                    verts = pare_output['verts']
                    logger.info(f"Extracted verts shape: {verts.shape}")
                else:
                    logger.error(f"Vertices not found in PARE output")
                    logger.error(f"Available keys: {pare_output.keys() if isinstance(pare_output, dict) else 'N/A'}")
                    return None
                
                # If multiple frames, take first frame
                if len(verts.shape) == 3:  # (n_frames, 6890, 3)
                    verts = verts[0]
                    logger.info(f"Took first frame, new shape: {verts.shape}")
                
                logger.info(f"Extracted vertices shape: {verts.shape}")
                return verts
    ...
    def _extract_measurements(
        ...
        logger.info(f"measurements_to_get: {measurements_to_get}")
        logger.info(f"user_height_cm: {user_height_cm}")
        
        try:
            # Initialize measurer if needed
            logger.info("[STEP 1/6] Initializing SMPL measurer...")
            self._init_smpl_measurer()
            logger.info("[STEP 1/6] ✓ SMPL measurer initialized")
            ...
            logger.info(f"[STEP 2/6] Converting vertices to tensor: {verts_tensor.shape}")
            
            # Load vertices into measurer
            try:
                logger.info(f"[STEP 3/6] Loading vertices into measurer...")
                self.smpl_measurer.from_verts(verts=verts_tensor)
                logger.info("✓ Vertices loaded successfully")
            except Exception as e:
            ...
            logger.debug(f"Cleared previous measurements")
            
            # Debug: Check if joints were calculated
            if hasattr(self.smpl_measurer, 'joints') and self.smpl_measurer.joints is not None:
                logger.info(f"✓ Joints calculated: shape={self.smpl_measurer.joints.shape}")
                logger.debug(f"  Joint regressor loaded: {self.smpl_measurer.joints is not None}")
            else:
                logger.error("✗ CRITICAL: Joints were not calculated by from_verts()")
                logger.error("This will cause all measurements to fail!")
                logger.error("Check if SMPL model is loaded correctly")
                logger.error(f"  Has 'joints' attribute: {hasattr(self.smpl_measurer, 'joints')}")
                if hasattr(self.smpl_measurer, 'joints'):
                    logger.error(f"  Joints value: {self.smpl_measurer.joints}")
                return {}
            
            # Debug: Check joint2ind dictionary
            if hasattr(self.smpl_measurer, 'joint2ind'):
                if 'spine1' in self.smpl_measurer.joint2ind:
                    logger.info(f"✓ 'spine1' is in joint2ind dict, index={self.smpl_measurer.joint2ind['spine1']}")
                else:
                    logger.warning("'spine1' is NOT in joint2ind dict!")
                    logger.info(f"Available joints in joint2ind: {list(self.smpl_measurer.joint2ind.keys())[:10]}")
            else:
                logger.error("✗ CRITICAL: joint2ind dictionary not found in measurer!")
            
            # Debug: Check if measurer has measurement_types
            if hasattr(self.smpl_measurer, 'measurement_types'):
                logger.info(f"✓ Measurement types available: {len(self.smpl_measurer.measurement_types)} types")
            else:
                logger.warning("measurement_types not found in measurer")
            
            # Get available measurements
            if not hasattr(self.smpl_measurer, 'all_possible_measurements'):
                logger.error("✗ CRITICAL: smpl_measurer.all_possible_measurements not found!")
                logger.error("SMPL measurer may not be initialized correctly")
                return {}
