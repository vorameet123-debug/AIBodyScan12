        ...
        for m_name in measurement_names:
            ...
            try:
                if self.measurement_types[m_name] == MeasurementType().LENGTH:
                    ...
                    print(f"Measurement {m_name} has unknown type")
...
class MeasureSMPL(Measurer):
    ...
    def __init__(self):
        ...
        # Use absolute path to data directory - DO NOT FALLBACK TO RELATIVE!
        # This ensures the path works regardless of where the API is running from
        ...
        if not os.path.exists(self.body_model_path):
            raise FileNotFoundError(
                f"SMPL data path not found at: {self.body_model_path}\n"
                f"Current file location: {__file__}\n"
                f"Body model root: {self.body_model_root}"
            )
...
class MeasureSMPLX(Measurer):
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.body_model_path):
            raise FileNotFoundError(
                f"SMPLX data path not found at: {self.body_model_path}\n"
                f"Current file location: {__file__}\n"
                f"Body model root: {self.body_model_root}"
            )
